...
export async function GET(request: NextRequest, context: RouteContext<any>) {
  const resolvedParams = await resolveRouteParams<any>(context);
  return withAuth(request, (req, user) => handleGET(req, user, resolvedParams));
}

export async function PUT(request: NextRequest, context: RouteContext<any>) {
  const resolvedParams = await resolveRouteParams<any>(context);
  return withAuth(request, (req, user) => handlePUT(req, user, resolvedParams));
}

export async function DELETE(request: NextRequest, context: RouteContext<any>) {
  const resolvedParams = await resolveRouteParams<any>(context);
